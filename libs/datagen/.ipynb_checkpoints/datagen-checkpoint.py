...
class Whole_Slide_Bag(Dataset):
    def __init__(self,
        ...
        self.new_path = ''
        if self.sp.dataset == 'TG':
            self.new_path = self.file_path.replace('/TG/data/open-tg-gates/', '/temp_tg/')
            os.makedirs(os.path.dirname(self.new_path), exist_ok=True)
            copyfile(self.file_path, self.new_path)
            print('Making file {}'.format(self.new_path))
            self.slide = utils.read_slide(self.new_path, self.sp)
            #print(self.slide.properties)
        else:
            self.new_path = self.file_path.replace('/PD/', '/temp_pd/')
            os.makedirs(os.path.dirname(self.new_path), exist_ok=True)
            copyfile(self.file_path, self.new_path)
            print('Making file {}'.format(self.new_path))
            self.slide = utils.read_slide(self.new_path, self.sp)
#             self.slide = utils.read_slide(self.file_path, self.sp)

        self.grids = grids
        self.size = int(np.round(tip.TILE_SIZE))
        self.level = 0
...
#         print('Idx: {}'.format(idx))
        ...
        img = utils.read_region(self.slide, coord, self.level, self.size, self.sp)
