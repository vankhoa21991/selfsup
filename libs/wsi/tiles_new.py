...
from . import util
from . import filter_new as filter
from . import slide_new as slide
...
def generate_tile_summaries(tile_sum, np_img, display=True, save_summary=False):
  ...
  summary_font = ImageFont.truetype(tp.SUMMARY_TITLE_FONT_PATH, size=tp.SUMMARY_TITLE_TEXT_SIZE)
  draw.text((5, 5), summary_txt, tp.SUMMARY_TITLE_TEXT_COLOR, font=summary_font)
  draw_orig.text((5, 5), summary_txt, tp.SUMMARY_TITLE_TEXT_COLOR, font=summary_font)

  if tp.DISPLAY_TILE_SUMMARY_LABELS:
    count = 0
    for t in tile_sum.tiles:
      count += 1
      label = "R%d\nC%d" % (t.r, t.c)
      font = ImageFont.truetype(tp.FONT_PATH, size=tp.TILE_LABEL_TEXT_SIZE)
      # drop shadow behind text
      draw.text(((t.c_s + 3), (t.r_s + 3 + z)), label, (0, 0, 0), font=font)
      draw_orig.text(((t.c_s + 3), (t.r_s + 3 + z)), label, (0, 0, 0), font=font)

      draw.text(((t.c_s + 2), (t.r_s + 2 + z)), label, tp.SUMMARY_TILE_TEXT_COLOR, font=font)
      draw_orig.text(((t.c_s + 2), (t.r_s + 2 + z)), label, tp.SUMMARY_TILE_TEXT_COLOR, font=font)
...
def generate_top_tile_summaries(tile_sum, np_img, display=True, save_summary=False, show_top_stats=True,
                                label_all_tiles=tp.LABEL_ALL_TILES_IN_TOP_TILE_SUMMARY,
                                border_all_tiles=tp.BORDER_ALL_TILES_IN_TOP_TILE_SUMMARY):
  """
  ...
  summary_title = "Slide %s Top Tile Summary:" % slide_num
  ...
  h_offset = tp.TILE_BORDER_SIZE + 2
  v_offset = tp.TILE_BORDER_SIZE
  h_ds_offset = tp.TILE_BORDER_SIZE + 3
  v_ds_offset = tp.TILE_BORDER_SIZE + 1
  for t in tiles_to_label:
    ...
    draw.text(((t.c_s + h_offset), (t.r_s + v_offset + z)), label, tp.SUMMARY_TILE_TEXT_COLOR, font=font)
    draw_orig.text(((t.c_s + h_offset), (t.r_s + v_offset + z)), label, tp.SUMMARY_TILE_TEXT_COLOR, font=font)
...
def np_tile_stat_img(tiles):
  ...
  np_stats = np_text(tile_stats, font_path=tp.SUMMARY_TITLE_FONT_PATH, font_size=14)
  return np_stats
...
def tile_border_color(tissue_percentage):
  ...
  """
  if tissue_percentage >= tp.TISSUE_HIGH_THRESH:
    border_color = tp.HIGH_COLOR
  elif (tissue_percentage >= tp.TISSUE_LOW_THRESH) and (tissue_percentage < tp.TISSUE_HIGH_THRESH):
    border_color = tp.MEDIUM_COLOR
  elif (tissue_percentage > 0) and (tissue_percentage < tp.TISSUE_LOW_THRESH):
    border_color = tp.LOW_COLOR
  else:
    border_color = tp.NONE_COLOR
  return border_color
...
def faded_tile_border_color(tissue_percentage):
  ...
  """
  if tissue_percentage >= tp.TISSUE_HIGH_THRESH:
    border_color = tp.FADED_THRESH_COLOR
  elif (tissue_percentage >= tp.TISSUE_LOW_THRESH) and (tissue_percentage < tp.TISSUE_HIGH_THRESH):
    border_color = tp.FADED_MEDIUM_COLOR
  elif (tissue_percentage > 0) and (tissue_percentage < tp.TISSUE_LOW_THRESH):
    border_color = tp.FADED_LOW_COLOR
  else:
    border_color = tp.FADED_NONE_COLOR
  return border_color
...
def summary_title(tile_summary):
  ...
  """
  return "Slide %s Tile Summary:" % tile_summary.slide_num
...
def summary_stats(tile_summary):
  ...
  return "Original Dimensions: %dx%d\n" % (tile_summary.orig_w, tile_summary.orig_h) + \
         ...
         " %5d (%5.2f%%) tiles >=%d%% tissue\n" % (
           tile_summary.high, tile_summary.high / tile_summary.count * 100, tp.TISSUE_HIGH_THRESH) + \
         " %5d (%5.2f%%) tiles >=%d%% and <%d%% tissue\n" % (
           tile_summary.medium, tile_summary.medium / tile_summary.count * 100, tp.TISSUE_LOW_THRESH,
           tp.TISSUE_HIGH_THRESH) + \
         " %5d (%5.2f%%) tiles >0%% and <%d%% tissue\n" % (
           tile_summary.low, tile_summary.low / tile_summary.count * 100, tp.TISSUE_LOW_THRESH) + \
         " %5d (%5.2f%%) tiles =0%% tissue" % (tile_summary.none, tile_summary.none / tile_summary.count * 100)


def tile_border(draw, r_s, r_e, c_s, c_e, color, border_size=tp.TILE_BORDER_SIZE, mode='outline'):
  """
  ...
  for x in range(0, border_size):
    if mode == 'outline':
      ...
      draw.rectangle([(c_s + x, r_s + x), (c_e - 1 - x, r_e - 1 - x)], fill=color)
...
def save_tile_summary_image(pil_img, slide_num):
  ...
  slide.save_thumbnail(pil_img, sp.THUMBNAIL_SIZE, thumbnail_filepath)
...
def summary_and_tiles(slide_num, display=True, save_summary=False, save_data=True, save_top_tiles=True, mpp='0.5024'):
  """
  ...
  tile_sum = score_tiles(slide_num, np_img, mpp=mpp)
  if save_data:
...
def score_tiles(slide_num, np_img=None, dimensions=None, small_tile_in_tile=False, mpp='0.5024'):
  """
  ...
  if str(mpp) == '0.2522' or str(mpp) == '0.2524':
    row_tile_size = round(tp.ROW_TILE_SIZE*2 / sp.SCALE_FACTOR)  # use round?
    col_tile_size = round(tp.COL_TILE_SIZE*2 / sp.SCALE_FACTOR)  # use round?
  elif str(mpp) == '0.5024' or str(mpp) == '0.5022' or str(mpp) == '0.4916' or str(mpp) == '0.4962' or str(mpp) == '0.4984':
    row_tile_size = round(tp.ROW_TILE_SIZE / sp.SCALE_FACTOR)  # use round?
    col_tile_size = round(tp.COL_TILE_SIZE / sp.SCALE_FACTOR)  # use round?
  else:
    print('mpp not supported: {}'.format(mpp))
  ...
  tile_sum = TileSummary(slide_num=slide_num,
                         orig_w=o_w,
                         orig_h=o_h,
                         orig_tile_w=tp.COL_TILE_SIZE,
                         orig_tile_h=tp.ROW_TILE_SIZE,
                         scaled_w=w,
                         scaled_h=h,
  ...
  for t in tile_indices:
    ...
    if (o_c_e - o_c_s) > tp.COL_TILE_SIZE:
      o_c_e -= 1
    if (o_r_e - o_r_s) > tp.ROW_TILE_SIZE:
      o_r_e -= 1
...
def multiprocess_filtered_images_to_tiles(display=False, save_summary=True, save_data=True, save_top_tiles=True,
  ...
  if save_summary and not os.path.exists(sp.TILE_SUMMARY_DIR):
...
def generate_tiled_html_result(slide_nums, tile_summaries_dict, data_link):
  ...
  if not sp.TILE_SUMMARY_PAGINATE:
    html = ""
    ...
    text_file = open(os.path.join(sp.TILE_SUMMARY_HTML_DIR, "tiles.html"), "w")
    text_file.write(html)
    text_file.close()
  else:
    total_len = len(slide_nums)
    page_size = sp.TILE_SUMMARY_PAGINATION_SIZE
...
def display_image(np_rgb, text=None, scale_up=False):
  ...
  """
  if scale_up:
    np_rgb = np.repeat(np_rgb, sp.SCALE_FACTOR, axis=1)
    np_rgb = np.repeat(np_rgb, sp.SCALE_FACTOR, axis=0)
...
def pil_text(text, w_border=tp.TILE_TEXT_W_BORDER, h_border=tp.TILE_TEXT_H_BORDER, font_path=tp.FONT_PATH,
             font_size=tp.TILE_TEXT_SIZE, text_color=tp.TILE_TEXT_COLOR, background=tp.TILE_TEXT_BACKGROUND_COLOR):
  """
...
def np_text(text, w_border=tp.TILE_TEXT_W_BORDER, h_border=tp.TILE_TEXT_H_BORDER, font_path=tp.FONT_PATH,
            font_size=tp.TILE_TEXT_SIZE, text_color=tp.TILE_TEXT_COLOR, background=tp.TILE_TEXT_BACKGROUND_COLOR):
  """
...
def hsv_purple_deviation(hsv_hues):
  ...
  """
  purple_deviation = np.sqrt(np.mean(np.abs(hsv_hues - tp.HSV_PURPLE) ** 2))
...
def hsv_pink_deviation(hsv_hues):
  ...
  """
  pink_deviation = np.sqrt(np.mean(np.abs(hsv_hues - tp.HSV_PINK) ** 2))
...
class TileSummary:
  ...
  def top_tiles(self):
    ...
    top_tiles = sorted_tiles[:tp.NUM_TOP_TILES]
    return top_tiles
