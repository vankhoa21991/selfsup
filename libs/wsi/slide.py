...
def get_training_slide_path(slide_number):
  ...
  slide_name = filedict[slide_number]
...
def get_tile_image_path(tile):
  ...
  tile_path = os.path.join(sp.TILE_DIR, padded_sl_num,
                           sp.TRAIN_PREFIX + padded_sl_num + "-" + sp.TILE_SUFFIX + "-r%d-c%d-x%d-y%d-w%d-h%d" % (
...
def get_tile_image_path_by_slide_row_col(slide_number, row, col):
  ...
  wilcard_path = os.path.join(sp.TILE_DIR, padded_sl_num,
                              sp.TRAIN_PREFIX + padded_sl_num + "-" + sp.TILE_SUFFIX + "-r%d-c%d-*." % (
...
def get_training_image_path(slide_number, large_w=None, large_h=None, small_w=None, small_h=None):
  ...
  if large_w is None and large_h is None and small_w is None and small_h is None:
    wildcard_path = os.path.join(sp.DEST_TRAIN_DIR, sp.TRAIN_PREFIX + padded_sl_num + "*." + sp.DEST_TRAIN_EXT)
    img_path = glob.glob(wildcard_path)[0]
  else:
    img_path = os.path.join(sp.DEST_TRAIN_DIR, sp.TRAIN_PREFIX + padded_sl_num + "-" + str(
...
def get_training_thumbnail_path(slide_number, large_w=None, large_h=None, small_w=None, small_h=None):
  ...
  padded_sl_num = str(slide_number).zfill(3)
  if large_w is None and large_h is None and small_w is None and small_h is None:
    wilcard_path = os.path.join(sp.DEST_TRAIN_THUMBNAIL_DIR, sp.TRAIN_PREFIX + padded_sl_num + "*." + sp.THUMBNAIL_EXT)
    img_path = glob.glob(wilcard_path)[0]
  else:
    img_path = os.path.join(sp.DEST_TRAIN_THUMBNAIL_DIR, sp.TRAIN_PREFIX + padded_sl_num + "-" + str(
      sp.SCALE_FACTOR) + "x-" + sp.DEST_TRAIN_SUFFIX + str(
...
def get_filter_image_filename(slide_number, filter_number, filter_name_info, thumbnail=False):
  ...
  img_filename = sp.TRAIN_PREFIX + padded_sl_num + "-" + padded_fi_num + "-" + sp.FILTER_SUFFIX + filter_name_info + "." + ext
  return img_filename
...
def get_tile_summary_image_filename(slide_number, thumbnail=False):
  ...
  img_filename = sp.TRAIN_PREFIX + padded_sl_num + "-" + str(sp.SCALE_FACTOR) + "x-" + str(large_w) + "x" + str(
...
def get_tile_data_filename(slide_number):
  ...
  training_img_path = get_training_image_path(slide_number)
  large_w, large_h, small_w, small_h = parse_dimensions_from_image_filename(training_img_path)
  data_filename = sp.TRAIN_PREFIX + padded_sl_num + "-" + str(sp.SCALE_FACTOR) + "x-" + str(large_w) + "x" + str(
...
def get_filter_image_result(slide_number):
  ...
  training_img_path = get_training_image_path(slide_number)
  large_w, large_h, small_w, small_h = parse_dimensions_from_image_filename(training_img_path)
  img_path = os.path.join(sp.FILTER_DIR, sp.TRAIN_PREFIX + padded_sl_num + "-" + str(
...
def get_filter_thumbnail_result(slide_number):
  ...
  training_img_path = get_training_image_path(slide_number)
  large_w, large_h, small_w, small_h = parse_dimensions_from_image_filename(training_img_path)
  img_path = os.path.join(sp.FILTER_THUMBNAIL_DIR, sp.TRAIN_PREFIX + padded_sl_num + "-" + str(
    sp.SCALE_FACTOR) + "x-" + sp.FILTER_SUFFIX + str(large_w) + "x" + str(large_h) + "-" + str(small_w) + "x" + str(
