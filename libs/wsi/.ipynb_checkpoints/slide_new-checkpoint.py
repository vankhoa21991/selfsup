...
import sys
from . import util
from .util import Time,vips2numpy
import torch
import cv2
from tqdm import tqdm

import libs.setting as st

sp = st.SlideParam()
filedict = torch.load(sp.file_mapping)
...
def get_training_slide_path(slide_name):
  ...
  #print('Filedict in slide'.format(filedict))
  slide_filepath = filedict[slide_name]
  # padded_sl_num = str(slide_number).zfill(3)
  # slide_filepath = os.path.join(sp.SRC_TRAIN_DIR, sp.TRAIN_PREFIX + padded_sl_num + "." + sp.SRC_TRAIN_EXT)
  
  # slide_filepath = os.path.join(sp.SRC_TRAIN_DIR,slide_name)
...
def get_tile_image_path(tile):
  ...
  tile_path = os.path.join(sp.TILE_DIR, padded_sl_num,
                           padded_sl_num + "-" + sp.TILE_SUFFIX + "-r%d-c%d-x%d-y%d-w%d-h%d" % (
                             t.r, t.c, t.o_c_s, t.o_r_s, t.o_c_e - t.o_c_s, t.o_r_e - t.o_r_s) + "." + sp.DEST_TRAIN_EXT)
  return tile_path
...
def get_tile_image_path_by_slide_row_col(slide_number, row, col):
  ...
  """
  padded_sl_num = str(slide_number).zfill(3)
  wilcard_path = os.path.join(sp.TILE_DIR, padded_sl_num,
                              padded_sl_num + "-" + sp.TILE_SUFFIX + "-r%d-c%d-*." % (
                                row, col) + sp.DEST_TRAIN_EXT)
  img_path = glob.glob(wilcard_path)[0]
  return img_path


def get_training_image_path(slide_name, large_w=None, large_h=None, small_w=None, small_h=None):
  """
  ...
  if large_w is None and large_h is None and small_w is None and small_h is None:
    wildcard_path = os.path.join(sp.DEST_TRAIN_DIR, slide_name + '-' + "*." + sp.DEST_TRAIN_EXT)
#     print(wildcard_path)
    img_path = glob.glob(wildcard_path)[0]
  else:
    img_path = os.path.join(sp.DEST_TRAIN_DIR, slide_name + "-" + str(
      sp.SCALE_FACTOR) + "x-" + sp.DEST_TRAIN_SUFFIX + str(
      large_w) + "x" + str(large_h) + "-" + str(small_w) + "x" + str(small_h) + "." + sp.DEST_TRAIN_EXT)
  return img_path


def get_training_thumbnail_path(slide_name, large_w=None, large_h=None, small_w=None, small_h=None):
  """
  ...
  # padded_sl_num = str(slide_number).zfill(3)
  if large_w is None and large_h is None and small_w is None and small_h is None:
    wilcard_path = os.path.join(sp.DEST_TRAIN_THUMBNAIL_DIR, slide_name+ "*." + sp.THUMBNAIL_EXT)
    img_path = glob.glob(wilcard_path)[0]
  else:
    img_path = os.path.join(sp.DEST_TRAIN_THUMBNAIL_DIR, slide_name + "-" + str(
      sp.SCALE_FACTOR) + "x-" + sp.DEST_TRAIN_SUFFIX + str(
      large_w) + "x" + str(large_h) + "-" + str(small_w) + "x" + str(small_h) + "." + sp.THUMBNAIL_EXT)
  return img_path
...
def get_filter_image_filename(slide_number, filter_number, filter_name_info, thumbnail=False):
  ...
  """
  if thumbnail:
    ext = sp.THUMBNAIL_EXT
  else:
    ext = sp.DEST_TRAIN_EXT
  padded_sl_num = str(slide_number).zfill(3)
  padded_fi_num = str(filter_number).zfill(3)
  img_filename = padded_sl_num + "-" + padded_fi_num + "-" + sp.FILTER_SUFFIX + filter_name_info + "." + ext
  return img_filename
...
def get_tile_summary_image_path(slide_number):
  ...
  """
  if not os.path.exists(sp.TILE_SUMMARY_DIR):
    os.makedirs(sp.TILE_SUMMARY_DIR)
  img_path = os.path.join(sp.TILE_SUMMARY_DIR, get_tile_summary_image_filename(slide_number))
  return img_path
...
def get_tile_summary_thumbnail_path(slide_number):
  ...
  """
  if not os.path.exists(sp.TILE_SUMMARY_THUMBNAIL_DIR):
    os.makedirs(sp.TILE_SUMMARY_THUMBNAIL_DIR)
  img_path = os.path.join(sp.TILE_SUMMARY_THUMBNAIL_DIR, get_tile_summary_image_filename(slide_number, thumbnail=True))
  return img_path
...
def get_tile_summary_on_original_image_path(slide_number):
  ...
  """
  if not os.path.exists(sp.TILE_SUMMARY_ON_ORIGINAL_DIR):
    os.makedirs(sp.TILE_SUMMARY_ON_ORIGINAL_DIR)
  img_path = os.path.join(sp.TILE_SUMMARY_ON_ORIGINAL_DIR, get_tile_summary_image_filename(slide_number))
  return img_path
...
def get_tile_summary_on_original_thumbnail_path(slide_number):
  ...
  """
  if not os.path.exists(sp.TILE_SUMMARY_ON_ORIGINAL_THUMBNAIL_DIR):
    os.makedirs(sp.TILE_SUMMARY_ON_ORIGINAL_THUMBNAIL_DIR)
  img_path = os.path.join(sp.TILE_SUMMARY_ON_ORIGINAL_THUMBNAIL_DIR,
...
def get_top_tiles_on_original_image_path(slide_number):
  ...
  """
  if not os.path.exists(sp.TOP_TILES_ON_ORIGINAL_DIR):
    os.makedirs(sp.TOP_TILES_ON_ORIGINAL_DIR)
  img_path = os.path.join(sp.TOP_TILES_ON_ORIGINAL_DIR, get_top_tiles_image_filename(slide_number))
  return img_path
...
def get_top_tiles_on_original_thumbnail_path(slide_number):
  ...
  """
  if not os.path.exists(sp.TOP_TILES_ON_ORIGINAL_THUMBNAIL_DIR):
    os.makedirs(sp.TOP_TILES_ON_ORIGINAL_THUMBNAIL_DIR)
  img_path = os.path.join(sp.TOP_TILES_ON_ORIGINAL_THUMBNAIL_DIR,
...
def get_tile_summary_image_filename(slide_number, thumbnail=False):
  ...
  img_filename = padded_sl_num + "-" + str(sp.SCALE_FACTOR) + "x-" + str(large_w) + "x" + str(
    large_h) + "-" + str(small_w) + "x" + str(small_h) + "-" + sp.TILE_SUMMARY_SUFFIX + "." + ext
...
def get_top_tiles_image_filename(slide_number, thumbnail=False):
  ...
  large_w, large_h, small_w, small_h = parse_dimensions_from_image_filename(training_img_path)
  img_filename = padded_sl_num + "-" + str(sp.SCALE_FACTOR) + "x-" + str(large_w) + "x" + str(
    large_h) + "-" + str(small_w) + "x" + str(small_h) + "-" + sp.TOP_TILES_SUFFIX + "." + ext
...
def get_top_tiles_image_path(slide_number):
  ...
  """
  if not os.path.exists(sp.TOP_TILES_DIR):
    os.makedirs(sp.TOP_TILES_DIR)
  img_path = os.path.join(sp.TOP_TILES_DIR, get_top_tiles_image_filename(slide_number))
  return img_path
...
def get_top_tiles_thumbnail_path(slide_number):
  ...
  """
  if not os.path.exists(sp.TOP_TILES_THUMBNAIL_DIR):
    os.makedirs(sp.TOP_TILES_THUMBNAIL_DIR)
  img_path = os.path.join(sp.TOP_TILES_THUMBNAIL_DIR, get_top_tiles_image_filename(slide_number, thumbnail=True))
  return img_path
...
def get_tile_data_filename(slide_number):
  ...
  training_img_path = get_training_image_path(slide_number)
  large_w, large_h, small_w, small_h = parse_dimensions_from_image_filename(training_img_path)
  data_filename = padded_sl_num + "-" + str(sp.SCALE_FACTOR) + "x-" + str(large_w) + "x" + str(
    large_h) + "-" + str(small_w) + "x" + str(small_h) + "-" + sp.TILE_DATA_SUFFIX + ".csv"
...
def get_tile_data_path(slide_number):
  ...
  """
  if not os.path.exists(sp.TILE_DATA_DIR):
    os.makedirs(sp.TILE_DATA_DIR)
  file_path = os.path.join(sp.TILE_DATA_DIR, get_tile_data_filename(slide_number))
  return file_path
...
def get_filter_image_result(slide_number):
  ...
  training_img_path = get_training_image_path(slide_number)
  large_w, large_h, small_w, small_h = parse_dimensions_from_image_filename(training_img_path)
  img_path = os.path.join(sp.FILTER_DIR, padded_sl_num + "-" + str(
    sp.SCALE_FACTOR) + "x-" + sp.FILTER_SUFFIX + str(large_w) + "x" + str(large_h) + "-" + str(small_w) + "x" + str(
    small_h) + "-" + sp.FILTER_RESULT_TEXT + "." + sp.DEST_TRAIN_EXT)
  return img_path
...
def get_filter_thumbnail_result(slide_number):
  ...
  training_img_path = get_training_image_path(slide_number)
  large_w, large_h, small_w, small_h = parse_dimensions_from_image_filename(training_img_path)
  img_path = os.path.join(sp.FILTER_THUMBNAIL_DIR, padded_sl_num + "-" + str(
    sp.SCALE_FACTOR) + "x-" + sp.FILTER_SUFFIX + str(large_w) + "x" + str(large_h) + "-" + str(small_w) + "x" + str(
    small_h) + "-" + sp.FILTER_RESULT_TEXT + "." + sp.THUMBNAIL_EXT)
  return img_path
...
def small_to_large_mapping(small_pixel, large_dimensions):
  ...
  large_x = round((large_w / sp.SCALE_FACTOR) / math.floor(large_w / sp.SCALE_FACTOR) * (sp.SCALE_FACTOR * small_x))
  large_y = round((large_h / sp.SCALE_FACTOR) / math.floor(large_h / sp.SCALE_FACTOR) * (sp.SCALE_FACTOR * small_y))
  return large_x, large_y


def training_slide_to_image(slide_name):
  """
  ...
  img, large_w, large_h, new_w, new_h = slide_to_scaled_pil_image(slide_name)

  img_path = get_training_image_path(slide_name, large_w, large_h, new_w, new_h)
  print("Saving image to: " + img_path)
  if not os.path.exists(sp.DEST_TRAIN_DIR):
    os.makedirs(sp.DEST_TRAIN_DIR)
  img.save(img_path)

  thumbnail_path = get_training_thumbnail_path(slide_name, large_w, large_h, new_w, new_h)
  save_thumbnail(img, sp.THUMBNAIL_SIZE, thumbnail_path)


def slide_to_scaled_pil_image(slide_name):
  """
  ...
  """
  slide_filepath = get_training_slide_path(slide_name)
  print("Opening Slide #%s: %s" % (slide_name, slide_filepath))
  '''
  Old approach
  '''
  ...
  if sp.mode == 'GCP':
      if sp.dataset == 'TG':
          new_path = slide_filepath.replace('/TG/data/open-tg-gates/', '/temp_tg/')
          os.makedirs(os.path.dirname(new_path), exist_ok=True)
          print('Copying {} to {}'.format(slide_filepath, new_path))
          copyfile(slide_filepath, new_path)
          slide = open_slide(new_path)
        
      elif sp.dataset == 'CS':
          new_path = slide_filepath.replace('/PD/', '/temp_pd/')
          os.makedirs(os.path.dirname(new_path), exist_ok=True)
          print('Copying {} to {}'.format(slide_filepath, new_path))
          copyfile(slide_filepath, new_path)
          slide = open_slide(new_path)
  ...
  new_w = math.floor(large_w / sp.SCALE_FACTOR)
  new_h = math.floor(large_h / sp.SCALE_FACTOR)
  level = slide.get_best_level_for_downsample(sp.SCALE_FACTOR)
  whole_slide_image = slide.read_region((0, 0), level, slide.level_dimensions[level])
  whole_slide_image = whole_slide_image.convert("RGB")
  img_pil = whole_slide_image.resize((new_w, new_h), PIL.Image.BILINEAR)
  ...
  # new_h = math.floor(large_h / sp.SCALE_FACTOR)
  # level = slide.get_best_level_for_downsample(sp.SCALE_FACTOR)
  # img = pyvips.Image.openslideload(slide_filepath, level=level)
  # np_3d = vips2numpy(img)
  # res = cv2.resize(np_3d, dsize=(new_w, new_h), interpolation=cv2.INTER_CUBIC)
  # img_pil = Image.fromarray(res).convert("RGB")

  return img_pil, large_w, large_h, new_w, new_h
...
def get_num_training_slides():
  ...
  """
  # num_training_slides = len(glob.glob(sp.SRC_TRAIN_DIR + "/**/*." + sp.SRC_TRAIN_EXT,recursive=True))
  num_training_slides = len(filedict)
...
def training_slide_range_to_images(start_ind, end_ind):
  ...
  """
  if os.path.isdir(sp.DEST_TRAIN_DIR):
        b = [i.split(sp.SRC_TRAIN_EXT)[0] + sp.SRC_TRAIN_EXT for i in os.listdir(sp.DEST_TRAIN_DIR)]
  else: b = []
  for slide_name in tqdm(filedict):
    if slide_name in b:
      continue
    training_slide_to_image(slide_name)
...
def slide_stats():
  ...
  if not os.path.exists(sp.STATS_DIR):
    os.makedirs(sp.STATS_DIR)
  ...
  stats_file = open(os.path.join(sp.STATS_DIR, "stats.txt"), "w")
  ...
  plt.set_cmap("prism")
  plt.tight_layout()
  plt.savefig(os.path.join(sp.STATS_DIR, "svs-image-sizes.png"))
  plt.show()
  ...
  plt.tight_layout()
  plt.savefig(os.path.join(sp.STATS_DIR, "svs-image-sizes-slide-numbers.png"))
  plt.show()
  ...
  plt.tight_layout()
  plt.savefig(os.path.join(sp.STATS_DIR, "distribution-of-svs-image-sizes.png"))
  plt.show()
  ...
  plt.tight_layout()
  plt.savefig(os.path.join(sp.STATS_DIR, "w-to-h.png"))
  plt.show()
  ...
  plt.tight_layout()
  plt.savefig(os.path.join(sp.STATS_DIR, "h-to-w.png"))
  plt.show()
